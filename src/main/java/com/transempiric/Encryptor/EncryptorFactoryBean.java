package com.transempiric.Encryptor;
 ...
 * @author Inenarratus
 */
public class EncryptorFactoryBean extends AbstractFactoryBean {
    public static final String ENCRYPTOR_SECRET_PROPERTY_NAME = "simple.encryptor.secret";
    public static final String ENCRYPTOR_SALT_PROPERTY_NAME = "simple.encryptor.salt";
    ...
    private static final String ENCRYPTOR_DEFAULT_SALT = "!I am not a beautiful and unique snowflake!";
    private static Log logger = LogFactory.getLog(EncryptorFactoryBean.class);
    ...
    public EncryptorFactoryBean() {
    }

    public EncryptorFactoryBean rsaEncryptor(final String path) {
    ...
    public EncryptorFactoryBean rsaDecryptor(final String path) {
    ...
    public EncryptorFactoryBean hexEncodingTextEncryptor(String secret, String salt) {
        if (salt == null) {
            salt = ENCRYPTOR_DEFAULT_SALT;
            logger.warn("Could not find " + ENCRYPTOR_SALT_PROPERTY_NAME
                    + " System property. Using ENCRYPTOR_DEFAULT_SALT.");
        }

        if (secret == null) {
            throw new SimpleEncryptorException("Could not find " + ENCRYPTOR_SECRET_PROPERTY_NAME
    ...
    private EncryptorFactoryBean createRsaEncryptor(final String data, String pattern) {
        if (data.contains(RSA_ANTI_PATTERN)) {
            throw new EncryptorException("Invalid key format for " + pattern + ". "
    ...
    private String getRawAsciiKey(final String path, String pattern) {
        ...
        try {
            ...
            throw new EncryptorException("Could not get " + pattern + " from file (" + path + ").", e);
        }
    ...
    private void doClearKeyFileContents() {
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                throw new EncryptorException("Could not clear key file contents (" + entry.getKey() + ").", e);
            } finally {
                it.remove();
            }
        }
    ...
    private void doDeleteKeyFilesl() {
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                throw new EncryptorException("Could not delete key file (" + entry.getKey() + ").", e);
            }
        }
    }
    ...
    public EncryptorFactoryBean deleteKeyFiles(boolean deleteKeyFiles) {
    ...
    public EncryptorFactoryBean clearKeyFileContents(boolean clearKeyFileContents) {
